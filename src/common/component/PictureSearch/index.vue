<template>
  <div class="picture-search" :class="size">
    ...
    <div class="append">
      <el-upload class="upload" action="" accept=".jpeg,.jpg,.png" :auto-upload="false" :show-file-list="false" :on-change="onUpload">
    ...
    <div class="search-list" :style="{top:$el.clientHeight+'px', width:$el.clientWidth+'px'}" v-if="init && async" v-show="listShow && asyncData.length">
      <div class="box">
        <div class="scroll">
          <ul>
            <li v-for="item in asyncData" :key="item.id" @click.stop="onItemChange(item)">
...
// import { postLabelSearch } from "@/api/globalSearch";
...
//   computed: {
//     ...mapGetters({
//       globalSearchData: "globalSearch/getData"
//     })
//   },
  mounted() {
    this.init = true;
  ...
  methods: {
    initData() {
      if (this.async && this.globalSearchData) {
        this.value = this.globalSearchData.name;
        this.image = this.globalSearchData.identityImg;
      }
    },
    onEmpty() {
      this.image = "";
    ...
    reset() {
      ...
      if (this.async) {
        this.$store.dispatch("globalSearch/setData", null);
      }
    }
  }
};
